...
def obter_secret(nome: str, default: str = "") -> str:
    ...
    return valor or os.getenv(nome, "") or env_local.get(nome, "") or default
...
def obter_chave_gemini() -> str:
    return limpar_chave(
        obter_secret("GEMINI_API_KEY")
        or obter_secret("GOOGLE_API_KEY")
    )
...
""".strip()
    ...
    try:
        ...
        # Se a biblioteca existe mas falhou por versão/configuração, tenta REST antes de desistir.
        erro_sdk_novo = str(exc)
    else:
        erro_sdk_novo = ""

    # 2) SDK antigo: google-generativeai
    try:
    ...
    # 3) REST direto: não depende de pacote Gemini instalado.
    try:
        ...
        detalhes = str(exc)
        extras = []
        if 'erro_sdk_novo' in locals() and erro_sdk_novo:
            extras.append(f"SDK novo: {erro_sdk_novo[:180]}")
        if 'erro_sdk_antigo' in locals() and erro_sdk_antigo:
...
with st.sidebar:
    ...
    if gemini_key:
        ...
        st.caption("Se a resposta ainda cair no modo local, confira internet, cota da API ou modelo informado.")
    else:
        st.warning("Sem chave Gemini. O app usa busca local no manual.")
        st.caption("Crie um arquivo .env com GEMINI_API_KEY=... ou use .streamlit/secrets.toml.")
